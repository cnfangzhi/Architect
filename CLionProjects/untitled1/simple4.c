...
// 4. 进阶，留一个思考
struct Worker1{// 定义一个结构体，相当于 java 的 class
    char name[18]; // 10
    int age; // 4
    double salary;// 8
    // char name[10] 24 ，char name[18]
    // 32
...
// 第三种方式，结构体嵌套
struct Worker2{// 定义一个结构体，相当于 java 的 class
	char name[10];
	int age;
	double salary;
...
// 第四种方式，结构体嵌套
struct Worker3{// 定义一个结构体，相当于 java 的 class
	char name[10];
	int age;
	double salary;
...
void test3(){
	struct Worker2 worker = { "Darren", 23, 1000, {"Android","中级"} };// 嵌套的初始
...
void test5(){
	...
	Worker* worker = (Worker*)malloc(sizeof(Worker1));
	worker->age = 24;
	strcpy_s(worker->name, "Jack");
...
typedef Worker1 Worker_;
// 对结构体的指针取别名
typedef Worker_* Worker0;

void test9(){
    Worker0 worker = (Worker_*)malloc(sizeof(Worker_));
